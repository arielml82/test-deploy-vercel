...
para lanzar `uvicorn server:app --reload --port 8000`.
...
El proyecto ya incluye `server.py`, que exporta la aplicación FastAPI, y un
`vercel.json` mínimo. Vercel detecta ese punto de entrada e instala las
dependencias desde `requirements.txt` automáticamente.

1. Sube el proyecto a un repositorio Git (no incluyas `.env`).
2. En Vercel, selecciona **Add New → Project** e importa el repositorio.
3. No configures un *Build Command*; Vercel reconoce FastAPI y Python.
4. Si defines variables, copia en Vercel los valores de `.env.example`.
5. Haz clic en **Deploy**. La documentación estará en `https://tu-dominio/docs`
   y el estado de la API en `https://tu-dominio/health`.
